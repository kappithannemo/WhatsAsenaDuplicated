...
const KingIsuwa = require('kingisuwa-public-2');
const chalk = require('chalk');
const axios = require('axios');
const config = require('./config');
const Heroku = require('heroku-client');
const {WAConnection, MessageOptions, MessageType, Mimetype, Presence} = require('@adiwajshing/baileys');
const {Message, StringSession, Image, Video} = require('./kingisuwa');
...
const QueenAmdiDB = config.DATABASE.define('KingIsuwa', {
    info: {
...
const KingIsuwaCon = new WAConnection();
        ...
        await axios.get('https://gist.githubusercontent.com/BlackAmda/c3877acdcdc041d77907d590d4ac1a2d/raw/').then(async (ann) => {
            ...
            if (infoen !== '' && config.LANG == 'EN') {
                while (getGMTh == 18 && getGMTm == 00) { 
                    return KingIsuwaCon.sendMessage(KingIsuwaCon.user.jid, '[ ```🔔King Isuwa Announcements🔔``` ]\n\n' + infoen.replace('{user}', QueenAmdiCon.user.name).replace('{wa_version}', QueenAmdiCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', QueenAmdiCon.user.phone.os_version).replace('{device_model}', QueenAmdiCon.user.phone.device_model).replace('{device_brand}', QueenAmdiCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
            else if (infosi !== '' && config.LANG == 'SI') {
                while (getGMTh == 18 && getGMTm == 00) { 
                    return KingIsuwaCon.sendMessage(KingIsuwaCon.user.jid, '[ ```🔔King Isuwa නිවේදන🔔``` ]\n\n' + infosi.replace('{user}', KingIsuwaCon.user.name).replace('{wa_version}', KingIsuwaCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', QueenAmdiCon.user.phone.os_version).replace('{device_model}', KingIsuwaCon.user.phone.device_model).replace('{device_brand}', KingIsuwaCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
        })
}, 50000);
...
setInterval(async () => { 
    if (config.AUTO_BIO == 'true') {
        var tz_bio = await QueenAmdi.timezone(KingIsuwaCon.user.jid)
        var date = await King Isuwa.datebio(config.LANG)
        const biography = '📅 ' + date + '\n⌚ ' + tz_bio + '    🎖️ ' + config.CAP
        await KingIsuwaCon.setStatus(biography)
    }
}, 7890);

/*
// --------------King isuwa verify system--------------
        ...
        await axios.get('https://gist.githubusercontent.com/BlackAmda/c3877acdcdc041d77907d590d4ac1a2d/raw/').then(async (ann) => {
            ...
            if (verify !== '' && config.LANG == 'EN') {
                while (getGMTh == 20 && getGMTm == 55) { 
                    return KingIsuwaCon.sendMessage(KingIsuwaCon.user.jid, HANDLER + verify.replace('{user}', KingIsuwaCon.user.name).replace('{wa_version}', KingIsuwaCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', KingIsuwaCon.user.phone.os_version).replace('{device_model}', KingIsuwaCon.user.phone.device_model).replace('{device_brand}', KingIsuwaCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
        ...
        await axios.get('https://gist.githubusercontent.com/BlackAmda/c3877acdcdc041d77907d590d4ac1a2d/raw/').then(async (ann) => {
            ...
            if (verify !== '' && config.LANG == 'EN') {
                while (getGMTh == 04 && getGMTm == 55) { 
                    return KingIsuwaCon.sendMessage(KingIsuwaCon.user.jid, HANDLER + verify.replace('{user}', QueenAmdiCon.user.name).replace('{wa_version}', KingIsuwaCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', KingIsuwaCon.user.phone.os_version).replace('{device_model}', KingIsuwaCon.user.phone.device_model).replace('{device_brand}', KingIsuwaCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
            else if (verify !== '' && config.LANG == 'SI') {
                while (getGMTh == 04 && getGMTm == 55) { 
...
if (config.WORKTYPE == 'public') {
    ...
    if (QueenAmdiCon.user.jid === '@s.whatsapp.net') {
        ...
        await KingIsuwaCon.sendMessage(KingIsuwaCon.user.jid, '*King Isuwa started in Private Mode👸*\n\n_Please do not try any commands here. This is your log number._\n_You can try commands anywhere else :)_\n\n_Type_ *.panel* _to get your full command list._\n\n_Your bot in Private Mode. To change, use_ ```.setvar WORK_TYPE=public``` _command._\n\n*You can check our website for new features : https://www.amdaniwasa.com/*\n\n*Comment here to request new features:*\nhttps://amdaniwasa.com/index-en.html#comments\n\n*Updates:* https://gist.github.com/BlackAmda/890b6b31fcb8d376d6a68afcb7359324\n\n *Thank you for using Queen Amdi 💌*', MessageType.text);
    }
}
    else {
    ...
    QueenAmdiCon.on('open', async () => {
        ...
        if (config.WORKTYPE == 'public') {
                ...
                if (QueenAmdiCon.user.jid === '@s.whatsapp.net') {
                    ...
                    await KingIsuwaCon.sendMessage(KingIsuwaCon.user.jid, '```🛡️ Blacklist අනාවරණය විය!``` \n```පරිශීලක:``` \n```හේතුව:``` ', MessageType.text)
